...
class ErrorControlledSparseSettingsV17:
    ...
    sparsity_audit_interval: int = 20
    max_audit_S_error: float = 0.010
    max_audit_H_error: float = 0.010
    max_audit_Snuc_error: float = 0.010
    audit_relaxation_factor: float = 0.5
    max_audit_relaxations: int = 4
    ...
    def validate(self):
        ...
        if self.sparsity_audit_interval<=0:
            raise ValueError("sparsity_audit_interval must be positive.")
        if not (0.0<self.audit_relaxation_factor<1.0):
            raise ValueError("audit_relaxation_factor must lie in (0,1).")
        if self.max_audit_relaxations<0:
            raise ValueError("max_audit_relaxations cannot be negative.")
        return self
...
def run_error_controlled_sparse_lvc_gaussians(
    initial_basis,
    C0=None,
    provider=None,
    grid=None,
    ...
    """v0.17 S/H/T edge-error-controlled sparse Gaussian propagation."""
    settings=settings.validate()
    provider=provider or AnalyticCI2DFrameProvider()
    if not isinstance(provider,AnalyticCI2DFrameProvider):
        raise TypeError(
            "v0.17 release runner is validated on the analytic 2-state LVC provider."
        )
    if grid is None:
    ...
    ledger=SparseErrorComplexityLedgerV17()
    ledger.start()
    ...
    def audit_and_relax(step,force=False):
        """Dense calibration checkpoint for the sparse S/H approximation.

        v0.17 deliberately uses occasional dense audits as a correctness bridge.
        Failed audits only relax the graph; thresholds are never tightened online.
        """
        nonlocal current_update,mats

        if (
            not force
            and step%settings.sparsity_audit_interval!=0
        ):
            return None

        attempts=0
        while True:
            ledger.dense_audits+=0
            ...
            passed=(
                ...
                <=settings.max_audit_S_error
                and audit["relative_H_frobenius_error"]
                <=settings.max_audit_H_error
                and audit["relative_Snuc_frobenius_error"]
                <=settings.max_audit_Snuc_error
            )
            ...
            if passed or attempts>=settings.max_audit_relaxations:
                if not passed:
                    events.append({
            ...
            endpoint_graph.relax_scores(
                settings.audit_relaxation_factor
            )
            midpoint_graph.relax_scores(
                settings.audit_relaxation_factor
            )
            endpoint_graph.relax_search_floor(
                settings.audit_relaxation_factor
            )
            midpoint_graph.relax_search_floor(
                settings.audit_relaxation_factor
            )
    ...
    if (
        not audit_history
        or audit_history[-1]["step"]!=int(steps)
    ):
        audit_and_relax(int(steps),force=True)
        renormalize()
    ...
    return {
        ...
        "settings":{
            ...
            "representation":
                "S/H/T edge-error-controlled sparse spinor-complete Gaussian basis",
            "solver":"scipy.sparse.linalg.spsolve",
            "locality":
                "safe geometric pre-screen + exact local S/H/T score + hysteresis + periodic dense audit",
            "adaptation":
