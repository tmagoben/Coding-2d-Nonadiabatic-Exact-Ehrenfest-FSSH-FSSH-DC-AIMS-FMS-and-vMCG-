...
$$
K_a(q)=\left(\partial_a H\right)_{\mathrm{op}}
=K_{a,{\mathrm{sf}}}(q)+K_{a,{\mathrm{SOC}}}(q).
$$
...
$$
F_{a,{\mathrm{SOC}}}=-\operatorname{Tr}\!\left[\rho K_{a,{\mathrm{SOC}}}\right].
$$

The validation fixes $\rho$ and independently checks this expression against the
centered finite difference of $-\operatorname{Tr}[\rho H_{\mathrm{SOC}}(q)]$.
...
The analytic $K_{q,{\mathrm{SOC}}}^{ST}$ is obtained by replacing $\lambda,\mu$ with
their exact derivatives. Setting all coupling gradients to zero therefore leaves a
nonzero constant $H_{\mathrm{SOC}}$ and exactly zero $K_{q,{\mathrm{SOC}}}$.
