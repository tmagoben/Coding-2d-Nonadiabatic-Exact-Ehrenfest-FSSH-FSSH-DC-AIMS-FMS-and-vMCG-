...
With nuclear Gaussian overlap $S_{ij}^{\mathrm{nuc}}$,

$$
\boxed{S_{ij}=S_{ij}^{\mathrm{nuc}}O_{ij}.}
$$
...
$$
\boxed{
T_{ij}^{(0)}=\tau_{ij}^{\mathrm{nuc}}O_{ij}+S_{ij}^{\mathrm{nuc}}O_{ij}\Gamma_j.
}
$$
...
$$
h_{ij}=\frac{\|H_{ij}\|_F/\sqrt s}{\max[\sqrt{E_iE_j},E_{\mathrm{floor}}]}.
$$

The nuclear moving-basis contribution is

$$
t_{ij}^{\mathrm{nuc}}=\Delta t\,
...
$$
\boxed{
\eta_{ij}^2=(w_Ss_{ij})^2+(w_Hh_{ij})^2+(w_tt_{ij}^{\mathrm{nuc}})^2+(w_mm_{ij})^2.
}
$$
...
$$
\eta_{ij}\ge\eta_{\mathrm{enter}},
$$

while existing edges persist until

$$
\eta_{ij}<\eta_{\mathrm{exit}}.
$$
